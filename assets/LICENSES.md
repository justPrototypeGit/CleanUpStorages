...
The upstream `LICENSE` file at the path above is the standard Apache License 2.0 text
and does not itself carry a filled-in per-repository copyright-holder line (the
Apache-2.0 template's `[yyyy] [name of copyright owner]` placeholder is left as
boilerplate); no separate `NOTICE` file exists in that repository either. The full
licence text follows verbatim.
