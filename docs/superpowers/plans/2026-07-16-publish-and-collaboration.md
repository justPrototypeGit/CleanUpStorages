...
- GitHub owner assumed to be **`justprototypelabs`**; Task 7 verifies before it is baked in anywhere user-visible.
...
repository = "https://github.com/justprototypelabs/CleanUpStorages"
readme = "README.md"
...
- Produces: a workflow named `CI` whose badge URL Task 6's README embeds:
  `https://github.com/justprototypelabs/CleanUpStorages/actions/workflows/ci.yml/badge.svg`
...
contact_links:
  - name: Design specs and implementation plans
    url: https://github.com/justprototypelabs/CleanUpStorages/tree/main/docs/superpowers
...
[![CI](https://github.com/justprototypelabs/CleanUpStorages/actions/workflows/ci.yml/badge.svg)](https://github.com/justprototypelabs/CleanUpStorages/actions/workflows/ci.yml)
...
```bash
git clone https://github.com/justprototypelabs/CleanUpStorages.git
cd CleanUpStorages
cargo build --release
```
...
Confirm all four:
1. The GitHub owner/username (the plan assumes `justprototypelabs` — if wrong, every URL written in Tasks 3–6 must be corrected first).
2. Repo name `CleanUpStorages`.
3. Public visibility.
4. ~~That the commit author email `[redacted]` becoming permanently public is accepted.~~
   **RESOLVED during execution:** rather than accept it, all 190 commits were rewritten to the
   GitHub noreply identity `217975680+justprototypelabs@users.noreply.github.com` before the first
...
**Confirmed by the user:** owner `justprototypelabs`, repo `CleanUpStorages`, remote
`https://github.com/justprototypelabs/CleanUpStorages.git`.
...
```bash
git remote add origin https://github.com/justprototypelabs/CleanUpStorages.git
```
...
Open `https://github.com/justprototypelabs/CleanUpStorages/actions` and confirm the `CI` run
...
Open `https://github.com/justprototypelabs/CleanUpStorages` and check: screenshots load, CI badge
